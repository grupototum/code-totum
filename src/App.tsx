...
import { ChevronLeft, ChevronRight, Code2, Brain, Settings, Zap, Bot } from 'lucide-react'
import Claudio from './pages/Claudio'
import Craudio from './pages/Craudio'
import Ada from './pages/Ada'
import SettingsPage from './pages/Settings'
import TotumAgent from './pages/TotumAgent'
...
export default function App() {
  ...
  useEffect(() => {
    if (location === '/') setLocation('/agent')
  }, [location, setLocation])

  const currentPage = location.split('/')[1] || 'agent'

  // TotumAgent tem layout próprio (full-screen, sem sidebar)
    ...
    <div className="h-screen bg-black dark text-white flex flex-col overflow-hidden" style={{ background: 'linear-gradient(135deg, #0a0a0a 0%, #1a0a2e 50%, #0f0f1e 100%)' }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} border-r border-white/10 backdrop-blur-md overflow-y-auto transition-all duration-300`} style={{ background: 'rgba(10, 10, 10, 0.6)' }}>
          <div className="p-4 space-y-6 flex flex-col h-full">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 hover:bg-white/10 rounded-lg transition text-gray-400 hover:text-white">
            ...
            <nav className="space-y-2 flex-1">
              {[
                { icon: Bot,    label: 'Totum Agent', path: '/agent',    active: false },
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-3 text-sm rounded-lg transition backdrop-blur-sm ${item.active ? 'text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
        ...
        <div className="flex-1 flex flex-col">
          {currentPage === 'claudio' ? <Claudio />
           : currentPage === 'craudio' ? <Craudio />
           : currentPage === 'ada' ? <Ada />
           : <SettingsPage />}
        </div>
      </div>
